...
class TPrivilegeObject;
class ThriftRpcHelper;

HttpHandlerWithAuth::HttpHandlerWithAuth(ExecEnv* exec_env, TPrivilegeObject::type object,
                                         TPrivilegeType::type type)
        : _exec_env(exec_env), _object(object), _type(type) {}
...
int HttpHandlerWithAuth::on_header(HttpRequest* req) {
    ...
    if (master_addr.hostname.empty() || master_addr.port == 0) {
        ...
        HttpChannel::send_error(req, SERVICE_UNAVAILABLE);
        return -1;
    }
    {
